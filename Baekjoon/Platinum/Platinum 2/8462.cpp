...
int main() {
    int n, m, k, mx = 0; cin >> n >> m; k = sqrt(n);
    vector<int> v(n);
    ...
    for (int i = 1; i < query.size(); i++) {
        ...
        while (_l < l) { l--; auto x = v[l - 1] - 1; sum += ((long long)b[x] * 2 + 1) * (x + 1); b[x]++; }
        while (_r > r) { r++; auto x = v[r - 1] - 1; sum += ((long long)b[x] * 2 + 1) * (x + 1); b[x]++; }
        while (_l > l) { auto x = v[l - 1] - 1; sum -= ((long long)b[x] * 2 - 1) * (x + 1); b[x]--; l++; }
        while (_r < r) { auto x = v[r - 1] - 1; sum -= ((long long)b[x] * 2 - 1) * (x + 1); b[x]--; r--; }
