...
void get()
{
    ...
    ll r = dt(pt[s[mnx]], pt[s[mxx]]);
    pos_t ap = pt[s[mnx]], bp = pt[s[mxx]];
    ...
    for (int i = 0; i < s.size(); i++) {
        ...
        ll tmp = dt(pt[s[mnx]], pt[s[mxx]]);
        if (r < tmp) {
            r = tmp; ap = pt[s[mnx]]; bp = pt[s[mxx]];
        }
    }
    
    cout << ap.x << " " << ap.y << " " << bp.x << " " << bp.y << "\n";
}
